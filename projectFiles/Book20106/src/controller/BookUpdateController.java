...
//	doProcess(req, resp) 에 request 값과 response 값을 파라미터 값으로 보내줌.
	...
	// 실제 BookUpdateController에서 Book을 update하기 위한 작업을 하는 메소드
...
//		instance 의 updateBook(vo) 메소드를 이용해서 실제 DB에 update 기능 구현.
...
//		cnt에 따라 다른 alert 창 띄우고 BookList.do 로 이동
		if(cnt > 0) {writer.println("<script>alert('도서 정보 수정 성공'); location.href='BookList.do'</script>");}
		else {writer.println("<script>alert('도서 정보 수정 실패'); location.href='BookList.do'</script>");}
